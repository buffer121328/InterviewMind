...
function Status({ status }: { status: string }) { const tone = status === 'succeeded' ? 'bg-emerald-50 text-emerald-700' : status === 'failed' ? 'bg-red-50 text-red-700' : status === 'pending_review' ? 'bg-amber-50 text-amber-700' : status === 'cancelled' ? 'bg-slate-100 text-slate-600' : 'bg-blue-50 text-blue-700'; const label = ({ queued: '排队中', running: '运行中', pending_review: '待人工复核', succeeded: '已通过', failed: '未通过', cancelled: '已取消' } as Record<string, string>)[status] ?? status; return <span className={`rounded-full px-2 py-0.5 text-[10px] ${tone}`}>{label}</span>; }
...
function formatDate(value: string): string { return formatChinaDateTime(value); }
