...
/** Coordinates the six evaluation-center surfaces without bypassing backend governance. */
export function EvaluationCenter() {
    ...
    const refresh = useCallback(async () => {
        setLoading(true);
        try {
            const [overviewResult, trendResult, regressionResult, datasetResult, suiteResult, runResult, queueResult, calibrationResult, gateResult] = await Promise.all([
                evaluationApi.overview(), evaluationApi.trends(), evaluationApi.regressions(), evaluationApi.datasets(), evaluationApi.suites(), evaluationApi.runs(), evaluationApi.annotationQueue(), evaluationApi.calibrations(), evaluationApi.gates(),
            ]);
    ...
    async function requestReview(runId: string) {
        try {
        ...
        <div className="mx-auto max-w-[1500px]">
            <header className="mb-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div className="flex gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-teal-50 text-teal-700"><ShieldCheck className="h-5 w-5" /></div><div><h2 className="text-xl font-semibold text-slate-900">Agent 评测中心</h2><p className="mt-1 text-sm text-slate-500">运行评测、观察质量、人工标注并校准 Agent 与 Judge</p><p className="mt-1 text-[11px] text-slate-400">PostgreSQL 保存生命周期事实；Langfuse 只承载 Trace 与 Score 镜像。</p></div></div>
                    <Button variant="outline" onClick={() => void refresh()} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? 'animate-spin' : ''}`} />刷新全部</Button>
                </div>
            </header>
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                <TabsList className="h-auto flex-wrap justify-start"><TabsTrigger value="overview">总览</TabsTrigger><TabsTrigger value="runs">评测运行</TabsTrigger><TabsTrigger value="datasets">数据集</TabsTrigger><TabsTrigger value="annotations">人工标注</TabsTrigger><TabsTrigger value="calibration">Judge 校准</TabsTrigger><TabsTrigger value="gates">发布门禁</TabsTrigger></TabsList>
                <TabsContent value="overview"><EvaluationOverviewPanel overview={overview} trends={trends} regressions={regressions} onOpenRun={openRun} onRequestReview={requestReview} /></TabsContent>
                <TabsContent value="runs"><EvaluationRunsPanel runs={runs} suites={suites} focusRunId={focusRunId} onRefresh={refresh} onOpenAnnotations={() => setActiveTab('annotations')} /></TabsContent>
