...
function ScoreSummary({ summary }: { summary: ReturnType<typeof summarizeEvaluationScores>[number] }) {
    const primaryOutcome = summary.failed > 0 ? 'failed' : summary.review > 0 ? 'review' : 'passed';
    return <div className={`rounded-md border p-2 ${toneClasses(primaryOutcome)}`}><div className="flex items-center justify-between gap-2 text-xs"><span className="font-semibold">{summary.label}</span><span>{summary.scores.length} 项</span></div><div className="mt-1.5 flex flex-wrap gap-1.5 text-[11px]"><span className="rounded bg-white/80 px-1.5 py-0.5">通过 {summary.passed}</span>{summary.failed > 0 && <span className="rounded bg-white/80 px-1.5 py-0.5">未通过 {summary.failed}</span>}{summary.review > 0 && <span className="rounded bg-white/80 px-1.5 py-0.5">待复核 {summary.review}</span>}{summary.hardGateFailures > 0 && <span className="rounded bg-red-100 px-1.5 py-0.5 text-red-800">硬门禁异常 {summary.hardGateFailures}</span>}</div></div>;
}
...
function ScoreDetail({ score }: { score: EvaluationScore }) {
    const outcome = evaluationScoreOutcome(score.status);
...
function toneClasses(outcome: ReturnType<typeof evaluationScoreOutcome>): string { return outcome === 'passed' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : outcome === 'failed' ? 'border-red-200 bg-red-50 text-red-800' : 'border-amber-200 bg-amber-50 text-amber-800'; }
