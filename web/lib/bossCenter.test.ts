...
test('getCaptureRunJobs only reads public captured-job summaries', () => {
    ...
    const run = {
        result: {
            jobs: [
                { job_id: 1, job_title: '后端工程师', company_name: '公司A', salary_text: '20k', city: '深圳', risk_flags: [] },
                { job_id: null, job_title: '前端工程师', company_name: '公司B', salary_text: '20k', city: '广州', risk_flags: [] },
                { job_id: 'oops', job_title: '坏数据', company_name: '公司C', salary_text: '1', city: '', risk_flags: [] },
            ],
        },
    } as never;
...
test('mergeAssetRun merges a child asset run result into its card', () => {
    ...
    assert.equal('greetings' in merged, false);
});
