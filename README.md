...
#### Agent 可观测性与评测
Langfuse 可关联 Agent、工具和模型调用，并支持 Prompt Management、环境/版本维度聚合、采样和 Scores；DeepEval 提供离线工具正确性检查与 LLM-as-Judge 质量评测。离线 DeepEval 断言在 pytest 中默认使用同步执行路径，避免混合 async 测试集产生 event loop deprecation warning；若启用 `LANGFUSE_EVAL_REPORTING_ENABLED=true`，成功断言后的 metric 会自动转换为 Langfuse Scores。相关开关和凭据统一在全量 `.env` 中声明。
...
- Redis 7（标准全量配置必需；使用 Docker Compose 时自动启动）
...
`env_example` 是唯一全量 `.env` 模板，包含数据库、Redis、队列、RAG、Embedding、语音、Langfuse、BOSS 和 mem0 的全部服务端变量。复制后不要删减任何变量；即使某个能力当前关闭，也保留对应开关和占位值，避免本地、API、Worker、Compose 之间配置漂移。

必须逐项确认并填写：

- PostgreSQL：`POSTGRES_PASSWORD` 与 `DATABASE_URL` 中的密码保持一致。
- AgentRun 加密：生成 API 与 Worker 共用的 `TASK_PAYLOAD_ENCRYPTION_KEY`。
- Redis / Worker：全量配置默认 `TASK_QUEUE_ENABLED=true`，本地和容器模式都启动 Redis 与 Worker。
- RAG Embedding：填写 `OPENAI_API_KEY`、`OPENAI_BASE_URL`、`EMBEDDING_MODEL`、`EMBEDDING_DIM`，维度必须与模型一致。
- Langfuse：保留并确认 `LANGFUSE_*` 全部变量；启用 tracing、prompt 或 eval 上报时填入真实密钥。
- BOSS：生成不少于 32 字符的 `BOSS_AUTOMATION_SERVICE_TOKEN`，API 与宿主机服务必须一致。
- mem0：确认 `MEM0_*` 全部变量；启用长期记忆时填写 LLM、Embedding 与 pgvector 连接。
- Web 地址：模板中的 `NEXT_PUBLIC_API_URL=/api` 用于全容器部署；本地直接运行前端时改为 `http://localhost:8000`。
...
该密钥会加密 PostgreSQL 中等待执行的 AgentRun 载荷；不要提交到仓库、日志、截图或示例请求。

> **说明：**
> - 本地运行后端时，`DATABASE_URL` 里的主机名用 `localhost`。
> - 使用 Docker Compose 时，Compose 会把 API/Worker 的 DB 和 Redis 地址覆盖为容器内服务名。
> - 前端模型配置也要完整校验：Smart、Fast、Voice、Reasoning Pool、Fast Pool 和简历专家通道都在前端「设置」中维护，模型 API Key 不写入服务器 `.env`。
> - `.env` 中所有变量都应保留；通过 `*_ENABLED` 开关控制能力是否运行，而不是删除变量。
...
全量配置默认 `TASK_QUEUE_ENABLED=true`：请保持 Redis 与 Worker 运行，首题、简历优化、报告和岗位资产统一走可恢复任务中心。`TASK_QUEUE_ENABLED=false` 仅用于极小范围单进程排障，不作为标准启动方式。
...
> 全容器模式保持 `NEXT_PUBLIC_API_URL=/api`，浏览器请求经同源 Nginx 转发，不要使用 `localhost:8000`。`docker-compose.yml` 通过 `env_file: .env` 把全量变量传入 migrate、backend、worker，并只覆盖容器内 `DATABASE_URL`、`REDIS_URL`、`AUTO_CREATE_TABLES` 和 mem0 的 PostgreSQL 地址。`TASK_QUEUE_ENABLED=true` 是标准配置，请保持 Worker 容器运行。首次建库和已有库升级步骤请以本 README 和当前迁移脚本为准；本地补充记录可放在不跟踪的 `docs/` 目录。
