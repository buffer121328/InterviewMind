...
📄 **建议从全量配置开始**：先阅读 [启动与配置指南](docs/配置说明.md#1-全量配置先复制这一份)，复制完整 `env_example` 后再启动；第一次运行和前端逐项验收请见 [求职者端配置、启动与功能验收指南](docs/求职者端配置启动与功能验收指南.md)。
...
| `POST /api/agent-runs/{run_id}/cancel` | 取消等待中的任务 |
