...
📄 **建议从全量配置开始**：先阅读 [启动与配置指南](docs/配置说明.md#1-全量配置先复制这一份)，复制完整 `env_example` 后再启动；如果想按按钮/API/后端函数快速上手，请看 [前后端功能链路学习地图](docs/配置说明.md#9-前后端功能链路学习地图)；第一次运行和前端逐项验收请见 [求职者端配置、启动与功能验收指南](docs/求职者端配置启动与功能验收指南.md)。
...
- 前后端功能链路学习地图：`docs/配置说明.md#9-前后端功能链路学习地图`
...
`env_example` 已包含数据库、队列、RAG、语音、Langfuse、BOSS 和 mem0 的全量变量。至少修改数据库密码、`DATABASE_URL`、`NEXT_PUBLIC_API_URL` 和队列加密密钥，详见 [全量配置说明](docs/配置说明.md#1-全量配置先复制这一份)。
...
> 全容器模式建议设置 `NEXT_PUBLIC_API_URL=http://localhost`。首次建库和已有库升级的完整顺序见 [启动与配置指南](docs/配置说明.md)。
