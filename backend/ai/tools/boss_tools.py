...
async def save_job_to_database(raw_data: Dict[str, Any], user_id: str,
    ...
    """标准化并保存岗位到数据库"""
    from integrations.boss.job_normalizer import normalize_company_name, normalize_salary, extract_keywords, compute_source_hash
...
async def generate_job_assets(job_id: int, user_id: str, resume_content: str,
    ...
    """为指定岗位生成投递资产"""
    from ai.workflows.jobs_support.job_asset_orchestrator import generate_assets as _gen
    try:
