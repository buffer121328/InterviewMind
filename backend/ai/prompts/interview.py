...
5. 如果规划上下文中的上一轮短板/未解决短板非空，至少一道主问题必须直接覆盖其中一个短板或其明确等价表达；只能改变提问角度，不能把短板要求改成泛化题。没有短板时不得强行插入固定短板题。
6. sources 只引用真实提供的来源；没有来源时使用空数组，fallback_reason 说明为何使用通用题。
7. id 从 1 连续编号，type 只能是 intro、tech、behavior 或 system_design。
8. content 只写候选人可见的问题，不要在问题正文提前给答案、提示或评价；不得要求绘制完整组件图、数据流图、流程图或提交书面材料作为作答前置条件，系统设计题改为口头说明思路、取舍和关键约束。
9. answer_points 是内部辅导数据，每题提供 2-4 条简洁中文要点；只保留必要技术专有名词原文，不得编造候选人经历。
