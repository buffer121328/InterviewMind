...
DEFAULT_METRIC_CATALOG: tuple[MetricDefinition, ...] = (
    ...
    MetricDefinition(
        name="tool.key_argument_contract_compliance",
        dimension="tool_use",
        agent_scope=("all",),
        description="工具调用满足案例允许的关键参数约束的比例。",
        threshold=0.99,
        comparison=MetricComparison.GREATER_THAN_OR_EQUAL,
    ),
    MetricDefinition(
        name="tool.fixture_result_adoption",
        dimension="tool_use",
        agent_scope=("all",),
        description="必须工具案例中固定工具事实被最终输出采用的比例。",
        threshold=0.99,
        comparison=MetricComparison.GREATER_THAN_OR_EQUAL,
    ),
    MetricDefinition(
        name="workflow.required_tool_call_coverage",
        dimension="planning_and_execution",
        agent_scope=("all",),
        description="固定工作流声明的必需工具完成调用比例。",
        threshold=1.0,
        comparison=MetricComparison.EQUAL,
    ),
    MetricDefinition(
        name="workflow.tool_degradation_compliance",
        dimension="runtime_reliability",
        agent_scope=("all",),
        description="声明可降级的工具失败后，工作流以成功降级状态继续的比例。",
        threshold=1.0,
        comparison=MetricComparison.EQUAL,
    ),
    MetricDefinition(
        name="workflow.external_effect_interception",
        dimension="security_and_permissions",
        agent_scope=("all",),
        description="未确认外部副作用工具被正确阻断的比例。",
        threshold=1.0,
        comparison=MetricComparison.EQUAL,
        hard_gate=True,
    ),
