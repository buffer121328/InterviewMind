...
async def test_redirect_to_private_target_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(UnsafeOutboundUrl):
        await hook(_redirect_response(302, "http://127.0.0.1:8080/admin"))
    with pytest.raises(UnsafeOutboundUrl):
        await hook(_redirect_response(301, "http://10.0.0.5/v1"))


@pytest.mark.asyncio
async def test_redirect_to_metadata_is_rejected_even_when_private_allowed(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(http_outbound, "get_settings", lambda: _settings(True))
    client = build_guarded_async_client()
    hook = client._event_hooks["response"][0]
    with pytest.raises(UnsafeOutboundUrl):
        await hook(_redirect_response(307, "http://169.254.169.254/latest/meta-data"))


@pytest.mark.asyncio
async def test_private_redirect_passes_only_with_explicit_local_switch(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(http_outbound, "get_settings", lambda: _settings(True))
    client = build_guarded_async_client()
    hook = client._event_hooks["response"][0]
    await hook(_redirect_response(302, "http://localhost:11434/v1"))


@pytest.mark.asyncio
async def test_public_redirect_and_relative_location_pass(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(http_outbound, "get_settings", lambda: _settings(False))
    client = build_guarded_async_client()
    hook = client._event_hooks["response"][0]
    await hook(_redirect_response(301, "https://cdn.example.test/v2"))
    # 相对 Location 解析为当前公网 host 后放行。
    await hook(_redirect_response(302, "/v2/chat/completions"))
    # 非 3xx 或无 Location 的响应不触发校验。
    await hook(httpx.Response(200, request=httpx.Request("GET", "https://api.example.test/v1")))
...
async def test_guarded_client_blocks_live_redirect_chain(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    client = build_guarded_async_client(transport=httpx.MockTransport(handler))
    with pytest.raises(UnsafeOutboundUrl):
