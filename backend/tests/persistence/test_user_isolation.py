...
class TestSessionIsolation:
    ...
    async def test_user_a_cannot_get_user_b_session(self):
        """用户A不能获取用户B的会话"""
        with patch(
            "app.db.repositories.session.repo_impl.session_mgmt.SessionManagementService.get_session"
        ) as mock_get:
    ...
    async def test_list_sessions_filtered_by_user(self):
        """列表查询按 user_id 过滤"""
        with patch(
            "app.db.repositories.session.repo_impl.session_mgmt.SessionManagementService.list_sessions"
        ) as mock_list:
...
class TestProfileIsolation:
    ...
    async def test_user_profile_scoped(self):
        """用户画像按 user_id 隔离"""
        with patch(
            "app.db.repositories.session.repo_impl.profile_mgmt.ProfileService.get_user_profile"
        ) as mock_get:
...
class TestWeaknessReportIsolation:
    ...
    async def test_weakness_report_by_user(self):
        """短板报告查询需要 user_id"""
        with patch(
            "app.db.repositories.interview.weakness_report_repo.get_weakness_report_repo"
        ) as mock_repo:
...
class TestInterviewStateUserId:
    ...
    def test_state_has_user_id(self):
        ...
        from ai.agents.interview.interview_graph import InterviewState
...
class TestNextRoundUserIdPropagation:
    ...
    async def test_next_round_inherits_parent_user_id(self):
        ...
        with patch(
            "app.db.repositories.session.repo_impl.session_advanced.SessionAdvancedService.create_next_round"
        ) as mock_create:
            mock_session = MagicMock()
