...
def test_interview_report_profile_uses_project_utc_clock(monkeypatch):
    ...
    from ai.workflows.analysis import report_records
    from app.schemas.llm_outputs import CandidateProfileOutput

    fixed_now = datetime(2026, 8, 4, 18, 45, 0)
    monkeypatch.setattr(report_records, "utc_now", lambda: fixed_now)
    dimension = {
        "score": 7.0,
        "evidence": "一组真实问答证据",
        "reason": "证据有限",
    ...
    profile = report_records.to_candidate_profile(
        result,
        total_questions=1,
    )
