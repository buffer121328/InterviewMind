        ...
        "operations": [
            {
                "memory_id": "memory-1",
                "action": "UPDATE",
                "content": "类别：技术偏好；事实：用户偏好使用 FastAPI；依据：用户明确说明。",
                "confidence": 0.95,
...
async def test_historical_consolidation_dry_run_and_confirmed_apply():
        ...
        "operations": [
            {
                "memory_id": "memory-1",
                "action": "UPDATE",
                "content": "类别：技术偏好；事实：用户偏好并主要使用 FastAPI；依据：用户明确说明。",
                "confidence": 0.96,
    ...
    assert memory.records["memory-1"]["memory"] == "类别：技术偏好；事实：用户偏好并主要使用 FastAPI；依据：用户明确说明。"


def test_historical_plan_requires_structured_chinese_update_content():
    """已有记忆整理的 UPDATE 必须是结构化中文，避免自由文本继续污染存量记忆。"""
    payload = {
        "operations": [
            {
                "memory_id": "memory-1",
            ...
            },
            {
                "memory_id": "memory-2",
                "action": "UPDATE",
                "content": "类别：技术偏好；事实：用户偏好使用 FastAPI 开发后端服务；依据：用户明确说明。",
                "confidence": 0.97,
                "reason": "结构化中文改写",
            },
        ]
    ...
    plan = parse_historical_plan(
        ...
        owned_ids={"memory-1", "memory-2"},
    )

    assert {operation.memory_id: operation.action for operation in plan} == {
        "memory-1": ConsolidationAction.KEEP,
        "memory-2": ConsolidationAction.UPDATE,
    }
    assert next(operation for operation in plan if operation.memory_id == "memory-2").content == (
        "类别：技术偏好；事实：用户偏好使用 FastAPI 开发后端服务；依据：用户明确说明。"
    )


def test_historical_group_rejects_unstructured_canonical_content():
    """合并组的规范内容缺少结构化字段时，不应更新规范记忆或删除片段。"""
    payload = {
        "groups": [
            {
                "canonical_id": "memory-1",
                "source_ids": ["memory-1", "memory-2"],
                "content": "用户偏好使用 FastAPI，并主要做后端开发。",
                "confidence": 0.98,
                "reason": "同主题合并",
            }
        ]
    ...
    assert {operation.memory_id: operation.action for operation in plan} == {
        "memory-1": ConsolidationAction.KEEP,
        "memory-2": ConsolidationAction.KEEP,
    }


@pytest.mark.asyncio
async def test_historical_consolidation_applies_structured_chinese_and_preserves_fragments_on_update_failure():
    """确认整理时写入结构化中文；规范更新失败时，不删除依赖该更新的片段。"""
    records = [
        {"id": "memory-1", "memory": "User prefers FastAPI for backend development", "metadata": {}},
        {"id": "memory-2", "memory": "用户主要使用 FastAPI", "metadata": {}},
    ]
    payload = {
        "groups": [
            {
                "canonical_id": "memory-1",
                "source_ids": ["memory-1", "memory-2"],
                "content": "类别：技术偏好；事实：用户偏好并主要使用 FastAPI 开发后端服务；依据：用户明确说明。",
                "confidence": 0.98,
                "reason": "同主题结构化合并",
            }
        ]
    ...
    assert preview["counts"] == {"KEEP": 0, "UPDATE": 1, "DELETE": 1}
    assert memory.operations == []
    ...
    assert applied["applied_counts"] == {"UPDATE": 1, "DELETE": 1}
    assert memory.records == {
        "memory-1": {
            "id": "memory-1",
            "memory": "类别：技术偏好；事实：用户偏好并主要使用 FastAPI 开发后端服务；依据：用户明确说明。",
            "metadata": {},
        }
    }

    failing_memory = _StatefulMemory(existing=records, add_result={"results": []}, lifecycle_payload=payload)

    def fail_update(*, memory_id, data, metadata=None):
        failing_memory.operations.append(("update", memory_id, data))
        return None

    failing_memory.update = fail_update
    failing_service = AgentMemoryService({"version": "test"})
    failing_service._memory = failing_memory
    failing_service._enabled = True

    failed_apply = await failing_service.consolidate_existing_memories(
        user_id="user-1",
        dry_run=False,
        max_memories=50,
    )

    assert failed_apply["applied_counts"] == {}
    assert set(failing_memory.records) == {"memory-1", "memory-2"}


def test_historical_prompt_requests_structured_chinese_for_existing_memories():
    """历史整理提示应明确要求把已有记忆改成类别/事实/依据结构。"""
    prompt = build_historical_consolidation_prompt(
        records=[{"id": "memory-1", "memory": "User prefers FastAPI", "metadata": {}}]
    )
