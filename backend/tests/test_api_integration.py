...
def _pre_mock():
    ...
    _sub_paths = [
        "app.repositories.session.repo_impl.base",
        "app.repositories.session.repo_impl.session_mgmt",
        "app.repositories.session.repo_impl.session_advanced",
        "app.repositories.session.repo_impl.message_mgmt",
        "app.repositories.session.repo_impl.profile_mgmt",
        "app.repositories.session.repo_impl.interview_plan",
    ]
    _names = [
    ...
    for path in _sub_paths:
        ...
        # 同时注入 backend.app.repositories... 路径
...
def _enter_test_client(app) -> TestClient:
    ...
    # 注意：lifespan 内通过 `from app.models import init_db` 导入，
    # 所以需要 patch app.models.init_db（而非 app.models.base.init_db）
    lifespan_patches = [
        patch("app.models.init_db", new_callable=AsyncMock),
        patch("app.services.agent_memory.get_agent_memory_service",
              new_callable=lambda: AsyncMock(return_value=_FakeMemoryService())),
        patch("app.services.agent_memory.close_agent_memory_service", new_callable=AsyncMock),
        patch("app.services.memory.close_checkpointer", new_callable=AsyncMock),
        patch("app.services.background_tasks.drain_background_tasks", new_callable=AsyncMock),
        patch("app.services.interview.interview_graph.clear_graph_instances", MagicMock),
    ]
