...
运行方式：
    cd backend
    uv run pytest tests/api/test_api_integration.py -v

只跑快速测试（不调 LLM）：
    uv run pytest tests/api/test_api_integration.py -v -m "fast"

只跑需要 LLM 的测试：
    uv run pytest tests/api/test_api_integration.py -v -m "llm"
"""
...
class TestHealthCheck:
    ...
    def test_unprefixed_documentation_routes_are_removed(self, client, path):
        """根路径文档兼容入口已退出，只保留统一的 /api 前缀。"""
    ...
    def test_prefixed_redoc_available(self, client):
        """GET /api/redoc returns ReDoc configured for the canonical schema URL."""
...
class TestOpenAPI:
    ...
    def test_openapi_json(self, client):
        """GET /api/openapi.json 返回有效 schema"""
        response = client.get("/api/openapi.json")
        assert response.status_code == 200
