...
async def test_boss_graph_uses_fixed_route_and_only_processes_top_n():
    ...
    with (
        patch("app.infrastructure.browser.boss_tools.check_environment", new=AsyncMock(return_value="✅ 环境正常")) as env,
        patch("app.infrastructure.browser.boss_tools.open_boss_search_page", new=AsyncMock(return_value="page")) as open_page,
        patch("app.infrastructure.browser.boss_tools.extract_job_cards_from_page", new=AsyncMock(return_value=cards)) as extract,
        patch("app.infrastructure.browser.boss_tools.score_jobs_by_match", new=AsyncMock(return_value=cards)) as score,
        patch("app.infrastructure.browser.boss_tools.save_job_to_database", new=AsyncMock(side_effect=[
            {"success": True, "job_id": 1, "is_duplicate": False},
            {"success": True, "job_id": 2, "is_duplicate": False},
        ])) as save,
        patch("app.infrastructure.browser.boss_tools.generate_job_assets", new=AsyncMock(return_value={"success": True})) as assets,
    ):
...
async def test_boss_graph_stops_after_environment_failure():
    ...
    with (
        patch("app.infrastructure.browser.boss_tools.check_environment", new=AsyncMock(return_value="环境问题: Chrome 未运行")),
        patch("app.infrastructure.browser.boss_tools.open_boss_search_page", new=AsyncMock()) as open_page,
    ):
