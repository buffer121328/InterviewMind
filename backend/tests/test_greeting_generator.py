...
def _message(suffix: str) -> str:
    """Build one valid first-person 100-220 character greeting fixture."""
    return (
...
class TestGreetingGenerator:
    ...
    def test_greeting_length_contract_rejects_oversized_text(self):
        """The structured contract rejects text over 220 characters."""
        from ai.agents.jobs.greeting_generator import GreetingItemOutput

        with pytest.raises(ValidationError):
            GreetingItemOutput(
                tone="professional",
                message_text="您好，" + "Java开发经验丰富，" * 50,
    ...
    async def test_fallback_greetings_are_first_person_and_complete(self):
        ...
        assert all(100 <= len(item["message_text"]) <= 220 for item in greetings)
