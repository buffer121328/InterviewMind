...
class ProfileService(BaseService):
    ...
    async def save_profile(
        ...
        """Save one session profile for its authenticated owner."""
        async with async_session() as db:
            try:
                ...
                result = await db.execute(
                    stmt.values(candidate_profile=profile_data, updated_at=datetime.now())
                )
                await db.commit()
    ...
    async def get_profile(
        ...
        """Read one session profile for its authenticated owner."""
        async with async_session() as db:
            stmt = select(SessionModel.candidate_profile).where(
                SessionModel.session_id == session_id,
                SessionModel.user_id == user_id,
            )
    ...
    async def get_series_final_profiles(self, limit: int, user_id: str) -> List[Dict[str, Any]]:
        ...
        async with async_session() as db:
            ...
            stmt = (
                select(SessionModel.candidate_profile)
                .where(SessionModel.candidate_profile.is_not(None))
                .where(~child_exists)
