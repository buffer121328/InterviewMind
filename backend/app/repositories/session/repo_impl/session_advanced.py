...
class SessionAdvancedService(BaseService):
    ...
    async def create_next_round(
        ...
        async with async_session() as db:
            db.add(SessionModel(
                session_id=new_session_id, user_id=user_id or "default_user", title=title, created_at=now, updated_at=now,
    ...
    async def clone_session_for_voice(
        ...
        async with async_session() as db:
            db.add(SessionModel(
                session_id=new_session_id, user_id=user_id or "default_user", title=title, created_at=now, updated_at=now, mode='voice',
