...
【输出 JSON 格式】必须是一个 JSON 对象，包含以下字段：
{
  "skill_match_score": 75,
  "skill_match_comment": "技能匹配评价",
  "project_match_score": 80,
  "project_match_comment": "项目匹配评价",
  "experience_match_score": 70,
  "experience_match_comment": "经验匹配评价",
  "education_match_score": 85,
  "education_match_comment": "教育匹配评价",
  "matched_keywords": ["Java", "Spring"],
  "missing_keywords": ["Kafka"],
  "strengths": ["优势1", "优势2"],
  "risks": ["风险1"],
  "priority_actions": ["改进动作1"],
  "selection_hints": {"key": "value"}
}

请严格以上述 JSON 对象结构输出，所有字段必须存在，不要省略字段，不要使用其它字段名。

Respond in JSON format."""


def build_user_prompt(resume_content: str, job_description: str) -> str:
    """构建用户 prompt（已经把 SYSTEM_PROMPT 合并进来，因为 invoke_structured 只接一个 prompt 字符串）"""
...
请严格按上面定义的 JSON 对象结构输出分析结果，所有 12 个字段必须存在，不要使用其它字段名。
