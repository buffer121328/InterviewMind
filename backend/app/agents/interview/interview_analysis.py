...
async def handle_interview_complete(
    ...
    """兼容旧调用；实际编排逻辑位于 ``app.workflows.interview.completion``。"""
    from app.workflows.interview.completion import handle_interview_complete as _handle_interview_complete

    return await _handle_interview_complete(
...
async def generate_session_reports(
    ...
    *,
    ...
    """兼容旧调用；实际编排逻辑位于 ``app.workflows.interview.completion``。"""
    from app.workflows.interview.completion import generate_session_reports as _generate_session_reports

    return await _generate_session_reports(
        session_id=session_id,
        api_config=api_config,
        user_id=user_id,
...
async def process_interview_summary(
    ...
    """兼容旧调用；实际总结编排逻辑位于 ``app.workflows.interview.completion``。"""
    from app.workflows.interview.completion import process_interview_summary as _process_interview_summary

    return await _process_interview_summary(
        session_id=session_id,
        messages=messages,
        mode=mode,
        api_config=api_config,
...
async def trigger_weakness_analysis(
    ...
    """
    try:
        from app.workflows.analysis.analysis_service import get_analysis_service, get_weakness_analysis_service
        from app.infrastructure.db.repositories.session.session_repo import SessionRepo
