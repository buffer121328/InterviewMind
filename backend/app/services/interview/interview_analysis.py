...
async def trigger_background_analysis(
    ...
    try:
        ...
        session = await session_repo.get_session(session_id, include_resume_content=True)
    ...
    try:
        ...
        if trigger_analysis:
            ...
            create_background_task(
                trigger_background_analysis(session_id, api_config),
...
async def trigger_weakness_analysis(
    ...
    try:
        ...
        logger.info(f"[WeaknessAnalysis] 开始触发短板地图分析，session_id: {session_id}")
        ...
        logger.info(f"[WeaknessAnalysis] 会话 {session_id} 的短板地图已生成并落库")
