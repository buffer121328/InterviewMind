...
DEPRECATED: 此模块已被 resume_orchestrator.py 的 6 阶段 Pipeline 替代。
新代码请使用 `resume_orchestrator.run_pipeline()` 入口。
此模块保留作为 fallback。

原有设计：8 节点（prepare + 3 parallel experts + moderator + reflect + refine + finalize）
新设计：6 阶段 Pipeline（JD分析 → 素材选择 → 定制改写 → 简历组装 → 事实核验 → 用户确认）
"""
# NOTE: This legacy fallback module is intentionally quiet at import time.
# Several compatibility tests and adapters need to import its types/functions;
# emitting a module-level DeprecationWarning made normal regression runs noisy.
# Keep the deprecation notice in this module docstring and prefer
# app.services.resume.resume_orchestrator.run_pipeline() for new code.
